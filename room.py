...
class Room:
    ...
    def gen_enemies(self, layer):
        # Calculate room dimensions based on your tilemap
        room_width = COLS * SCALE  # Total width in pixels
        room_height = (len(self.map['layers'][0]['data']) // COLS) * SCALE  # Total height in pixels
        ...
        for entity in layer['objects']:
            ...
            if entity['name'] == 'zombie':
                ...
                enemy = Enemy(textures.zombie, x, y, 70, 120, 30, 
                            animation, death_animation, room_width, room_height)
                self.enemies.append(enemy)

            elif entity['name'] == 'minion':
                animation = Animation(0, 3, 1, 0, 16, 0.2, 0.2, REPEATING, 32, 32)
                death_animation = Animation(0, 4, 1, 10, 16, .2, .2, ONESHOT, 32, 32)
                enemy = Enemy(textures.minion, x, y, 20, 200, 8, 
                            animation, death_animation, room_width, room_height)
                self.enemies.append(enemy)
                ...
                animation = Animation(0, 3, 1, 0, 16, 0.2, 0.2, REPEATING, 32, 32)
                death_animation = Animation(0, 4, 1, 10, 16, .2, .2, ONESHOT, 32, 32)
                enemy = Enemy(textures.mummy, x, y, 30, 135, 13, 
                            animation, death_animation, room_width, room_height)
                self.enemies.append(enemy)
                ...
                animation = Animation(0, 3, 1, 0, 16, 0.2, 0.2, REPEATING, 32, 32)
                death_animation = Animation(0, 4, 1, 10, 16, .2, .2, ONESHOT, 32, 32)
                enemy = Enemy(textures.bat, x, y, 40, 110, 20, 
                            animation, death_animation, room_width, room_height)
                self.enemies.append(enemy)
