...
from utilities import center_of_rect, vector2_subtract, vector2_length, vector2_normalize, Pathfinder
from collisions import check_obstacle_collisions, check_collision_line_rec
...
class Enemy:
    ...
    def __init__(self, sheet, x, y, hp, speed, dmg, animations, death_animation, world_width, world_height, attack_range=125, attack_width=150, show_radius=True, use_melee_attack=True):
        self.sheet = sheet
    ...
    def update_movement(self, player, obstacles):
        ...
        if dist_to_player <= self.detection_radius:
            # Check LOS and update LOS grace timer
            ...
            if self.path_update_cooldown <= 0 or not self.path:
                ...
                if new_path:
                    ...
                    # Try using previous player position as fallback if desired
                    self.last_reachable_pos = tgt  
            ...
            if self.path and self.current_waypoint < len(self.path):
                ...
                # Move toward last known reachable point
    ...
    def update_animation(self):
        ...
        if not self.is_alive:
            ...
            mapped = facing_map.get(self.facing, "front")
            desired = safe_get(f"attack_" + mapped)
    ...
    def draw(self):
        ...
        source = self.animation.animation_frame_horizontal() if self.animation else Rectangle(0, 0, 0, 0)
        ...
        draw_texture_pro(self.sheet, source, scaled, Vector2(0.0, 0.0), 0.0, WHITE)
    ...
    def draw_health_bar(self):
        ...
        y = self.rect.y - 12  # Just above the sprite
    ...
    def drop_item(self, room):
        ...
        if roll > 4:
            room.objects.append(Heart(x, y) if roll > 6 else Gold(x, y))
