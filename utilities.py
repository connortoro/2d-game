from raylibpy import Vector2, Rectangle, check_collision_point_rec, vector2_subtract, vector2_length, vector2_normalize
from raylibpy import Rectangle
import heapq
from typing import List, Tuple, Optional


def vector2_scale(vec: Vector2, scalar: float) -> Vector2:
...
def create_pathfinding_grid(obstacles: List[Rectangle], 
    ...
    for obstacle in obstacles:
        ...
        buffer = 1  # Number of cells to pad around obstacle
        for y in range(max(0, start_y - buffer), min(grid_height, end_y + 1 + buffer)):
            for x in range(max(0, start_x - buffer), min(grid_width, end_x + 1 + buffer)):
                grid[y][x] = 0
